...
function findLargerOrTie(a, b) {
  return a === b ? "tie" : a > b ? a : b; //return tie if numbers are equal,  if falsey, A is greater than B, so return A if not then we return B because it is falsey
}
...
//REMINDER: test are case sensative!
...
function animalCounts(animal, numberOfAnimals) {
  if (numberOfAnimals === 1) { //count of one for single animal 
    return numberOfAnimals + " " + animal // single form
  } else {
    return numberOfAnimals + " " + animal + "s" //plural
  }
}
...
function evenOrOddWithTernary(a) {
  ...
  return a % 2 !== 0 ? "Odd" :  a % 2 === 0 ? "Even" : "Odd"
  //rember to add the final falsey conditon, even if specified before
}
...
function findLargerTernary(a, b) {
  return (a > b) ? a : b //if a is greater than b, return a, if not return b
}
