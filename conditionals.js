//1. Write a block of code that checks and logs whether variables are equal or not.

let num = 20;
if (num > 9) {
  console.log("it's double digit");
} else {
  console.log("it's single digit");
}

//2. Write a block of code that logs which of two number variables is larger.
//Your output should look like this (in this example for the numbers x = 5 and y = 10):
// 'The greater number of 5 and 10 is 10'
//If the two numbers are equal, log: 'The numbers are equal
let x = 5;
let y = 10;
if (x < y) {
  console.log('y is greater than x');
} else {
  console.log('x >y is incorrect');
}

//3. Create a string variable with a language code (e.g. "english", "spanish", "german").
//Use an if - else if - else statement to log "Hello, World" in the given language,
//for at least 3 languages.
//It should default to logging Englis

let langs = 'Spanish';
if (langs === 'Spanish') {
  console.log('Hola Mundo');
} else if (langs === 'French') {
  console.log('Bojour Monde');
} else {
  console.log('Hello World');
}

//4. Do question 3 again but this time use a `switch` statement.
let lang = 'Spanish';
switch (lang) {
case 'English' :
  console.log('Hello World');
  break;
case 'Spanish' :
  console.log('Hola Mundo');
  break;
case 'French' :
  console.log('Bonjour Monde');
  break;
default:
  console.log('Greeting everone');
}

//5. Create a variable numGrade that will contain a numeric grade (out of 100).
//Use an if - else if ... - else statement to log the corresponding letter grade,
//either "A", "B", "C", "D", or "F"

let numGrade = '88';
if (numGrade < '100')
{
  console.log('you have an A');
} else if (numGrade < '90') {
  console.log('you have a B');
} else if (numGrade  < '80') {
  console.log('you have a C');
} else {
  console.log('you should take the course over');
}

//6. Create two variables: animalName ('cat', 'dog', etc.) , and numberOfAnimals.

//Use an if - else statement to log:

//* the number

//* either in single or plural form. Exp: "5 cats" or "1 dog"

let animalName = 'cat';
let numberOfAnimals = '7';
if (animalName === numberOfAnimals > 7)
  console.log(animalName + ' ' + numberOfAnimals);
else {
  console.log(numberOfAnimals + ' ' + animalName  + 's');
}

//7. Write a block of code that logs whether a given number is even or odd.

let number = '101';
if (number  % 2 === 0) {
  console.log(number + ' number is even');
} else {
  console.log(number + ' number is odd');
}

//8. Do question 7 again but use a ternary.
let numbers = 101;
numbers  % 2 ===   0 ? console.log(' number is even') : console.log('number is odd');

//9. Write a block of code that checks and logs whether
//a given number is positive, negative, or equal to 0.
let lastNum = 7;
if (lastNum > 0) {
  console.log(lastNum + ' number is positive');
}
   else if (lastNum < 0){
  console.log(lastNum + ' number is negative');
} else {
  console.log(' number = 0');
}
